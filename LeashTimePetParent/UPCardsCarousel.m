...
- (void)setupCardsView
{
    ...
    NSLog(@"set up cards view:%f,%f",self.frame.size.width,self.frame.size.height);
    ...
    if([object isEqual:_cardsContainer] && [keyPath isEqualToString:@"frame"]) {
        ...
        for(int i = 0; i < [_visibleCards count]; i++) {
            ...
            if(i < _visibleCardIndex) {
                ...
                //center = CGPointMake(40-card.frame.size.width/3.2, _cardsContainer.frame.size.height/3.2 +yOffset);
            } else {
                center = CGPointMake(10+_cardsContainer.frame.size.width/2, _cardsContainer.frame.size.height/2);
                
                //center = CGPointMake(card.frame.size.width/3.2, _cardsContainer.frame.size.height/3.2);
