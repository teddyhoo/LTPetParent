...
- (void)viewDidLoad {
    ...
    UIButton *backClearButton = [UIButton buttonWithType:UIButtonTypeCustom];
    backClearButton.frame = CGRectMake(5,5,32,32);
    [backClearButton setBackgroundImage:[UIImage imageNamed:@"back-button-blue-circle"] forState:UIControlStateNormal];
    [backClearButton addTarget:self action:@selector(backAndClear) forControlEvents:UIControlEventTouchUpInside];
    
    [self.view addSubview:background];
    
    _calendarContentView = [[JTHorizontalCalendarView alloc]initWithFrame:CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.height - 200)];
    [self.view addSubview:_calendarContentView];
    
    _calendarMenuView = [[JTCalendarMenuView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 100)];
    [self.view addSubview:_calendarMenuView];
    
    
    [_calendarMenuView addSubview:backClearButton];
    ...
    _addNoteView = [[AddNoteView alloc]initWithFrame:CGRectMake(self.view.frame.size.width, 120, self.view.frame.size.width, 300)];
...
-(void)backAndClear {
    ...
    for (int i = 0; i < [_sharedVisitsTracking.scheduleOfVisits count]; i++) {
        NSMutableDictionary *visitDic = [_sharedVisitsTracking.scheduleOfVisits objectAtIndex:i];
        NSLog(@"visitDic: %@",visitDic);
        visitDic = nil;
        [_sharedVisitsTracking.scheduleOfVisits removeObjectAtIndex:i];
    }
    for (int i = 0; i < [_datesSelected count]; i++) {
        NSLog(@"dates selected %@",[_datesSelected objectAtIndex:i]);
        [_datesSelected removeObjectAtIndex:i];
        
    }
    
    
    CGRect routeOptionsFrameOff = CGRectMake(_routeOptionsView.frame.size.width, _routeOptionsView.frame.origin.y, _routeOptionsView.frame.size.width, _routeOptionsView.frame.size.height);
    
    CGRect tableViewoff = CGRectMake(-1200, 0, _tableView.frame.size.width, _tableView.frame.size.height);
    ...
    [UIView animateWithDuration:0.4 delay:0.1 usingSpringWithDamping:0.5 initialSpringVelocity:0.8 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        _routeOptionsView.frame = routeOptionsFrameOff;
        
    } completion:^(BOOL finished) {
        
        _tableView.frame = tableViewoff;
    ...
    [_calendarContentView setFrame:CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.height - 400)];
    [_calendarMenuView setFrame:CGRectMake(0, 60, self.view.frame.size.width, 40)];
    
    [self didChangeModeTouch:[NSDate date]];
...
-(void)scheduleSettings {
    ...
    if ([_routeOptionsView.patternChosen isEqualToString:@"0"]) {
        ...
        for (NSDate *date in _datesSelected) {
            ...
            [visitSeries setObject:@"NONE" forKey:@"VisitNote"];
...
-(void) finishScheduleNote {
    ...
    CGRect noteOff = CGRectMake(_addNoteView.frame.origin.x-600, _addNoteView.frame.origin.y, _addNoteView.frame.size.width, _addNoteView.frame.size.height);
    CGRect tableViewOn = CGRectMake(0, 100, _tableView.frame.size.width, _tableView.frame.size.height);
...
- (void)didChangeModeTouch:(NSDate*)withDate
{
    ...
    NSLog(@"called did change mode touch");
    ...
    if(_calendarManager.settings.weekModeEnabled){
        newHeight = 85.;
        NSLog(@"week mode enabled");
        [_calendarContentView setFrame:CGRectMake(0, 90, self.view.frame.size.width, 70)];
    } else {
        [_calendarContentView setFrame:CGRectMake(0, 90, self.view.frame.size.width, self.view.frame.size.height - 400)];
